...
export class DailyHeikinNotifier extends BaseStrategy {
  ...
  async realtimeAdvice(candle: Candle) {
    ...
    if (!this.hasRequested || this.hasRequested?.candles[this.hasRequested.candles.length - 1].time.getDay() != day) {
      if (hour > 13) {
        this.called++;
        console.log(this.called);
        this.hasRequested = await this.strat.exchange.getHistoryWithIndicator(this.ticker.pair, '12h');
        const lastHeikin: Candle = this.hasRequested.heikin.candles[this.hasRequested.heikin.candles.length - 2];
