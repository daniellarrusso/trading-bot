...
export class PortfolioTrades {
  ...
  candle: Candle;
  timeSync: TimeSync;
  constructor(public strategy: Strategy, public stats: CandleStatistics) {
    this.exchange = strategy.exchange;
  ...
  addTrade(percentage: Percentage) {
    ...
    let buybackPrice = sellOrder.price - (sellOrder.price - this.stats.getHighLowForPeriod(21, false)) * percentage.fibRatio;
  ...
  updateBuyPrices(trades: PortfolioTrade[]) {
    ...
    const lowPrice = this.strategy.portfolio.buyPrice ? this.strategy.portfolio.buyPrice : this.stats.getHighLowForPeriod(21, false);
    trades.map((t) => {
  ...
  calculateSellPrice(): number {
    ...
    if (buyPriceSet) {
      ...
      result = Math.max(this.stats.getHighLowForPeriod(21, false), this.strategy.exchange.ticker.candle.close);
    }
    return result;
  }
  ...
  async triggerLimitOrder(order: LimitOrder) {
    ...
    try {
      ...
      let message = `${order.side.toUpperCase()} Limit order triggered for ${order.quantity} ${this.exchange.ticker.pair
