...
export class PortfolioTrades {
    ...
    addTrade(percentage: Percentage) {
        const newTrade = new PortfolioTrade(
            ++this.tradeId,
            this.strategy.exchange.ticker.tickSize,
            percentage
        );
        ...
        let buybackPrice =
            sellOrder.price -
            (sellOrder.price - this.stats.getHighLowForPeriod(21, false)) * percentage.fibRatio;
    ...
    updateBuyPrices(trades: PortfolioTrade[]) {
        ...
        const lowPrice = this.strategy.portfolio.buyPrice
            ? this.strategy.portfolio.buyPrice
