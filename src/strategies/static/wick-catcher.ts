...
export class WickCatcherStrategy extends BaseStrategy {
  ...
  async realtimeAdvice(candle: Candle) {
    ...
    if (+priceIncrease > 1) console.log(`${this.ticker.pair} Price Increase: ${priceIncrease.toFixed(2)}%`);
  ...
  async advice() {
    ...
    const volIncrease = this.returnPercentageIncrease(this.candle.volume, this.previousCandle.volume).toFixed(2);
