...
async function writeBinancePairs() {
  const { data: markets } = await gecko.getMarkets({ per_page: 200, order: 'volume_desc' });
  const strats = [];
  let ranker = 1;
  for (let i = 0; i < markets.length; i++) {
    const res = await someFunction(markets[i].id, { exchange_ids: 'binance' });

    const btcPair = res.find((e) => e.target === 'USDT' && !excludes.some((b) => b === e.base));
    if (btcPair) {
      strats.push({ rank: ranker++, asset: btcPair.base, currency: btcPair.target });
    }
  }
...
async function writeEthPairs() {
  const { data: markets } = await gecko.getMarkets({ per_page: 20, order: 'volume_desc' });
  const strats = [];
  let ranker = 1;
  for (let i = 0; i < markets.length; i++) {
    const res = await someFunction(markets[i].id, { exchange_ids: 'binance' });
    const btcPair = res.find((e) => e.target === 'ETH' && !excludes.some((b) => b === e.base));
    if (btcPair) {
